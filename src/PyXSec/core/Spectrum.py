...
class Spectrum:
    ...
    def _load_histograms(self):
        ...
        f_temp = TFile.Open(self.input_data_path)
        h_temp = None
        f_temp.cd()
        h_temp = gDirectory.Get(self.histo_data_path)
        if self.do_total_cross_section == 0:
            self.h_data = h_temp.Clone()
        else:
            self.h_data = TH1D("h_data", "data", 1, 0, 1)
            integral, error = 0, 0
        ...
        f_temp = TFile.Open(self.input_sig_path)
        f_temp.cd()
        h_temp = gDirectory.Get(self.histo_reco_path)
        if self.do_total_cross_section == 0:
            self.h_signal_reco = h_temp.Clone()
        else:
            self.h_signal_reco = TH1D("h_signalReco", "signalReco", 1, 0, 1)
            integral, error = 0, 0
            integral = h_temp.IntegralAndError(1, h_temp.GetNbinsX(), error)
        ...
        f_temp = TFile.Open(self.input_res_path)
        f_temp.cd()
        h_response_temp = gDirectory.Get(self.histo_res_path)
        if self.do_total_cross_section == 0:
            ...
            if label == "":  # this means that we are doing a 2D unfolding
                ...
                self.h_response = h_response_temp.Clone("response_tmp")
                # self.h_response = TH2D(
                #     "response_tmp",
                #     "response_tmp",
            ...
            self.h_response = TH2D("response", "response", 1, 0, 1, 1, 0, 1)
            integral, error = 0, 0
        ...
        h_response_temp.SetDirectory(0)
        f_temp.Close()
        ...
        if self.do_efficiency_correction == 1:
            ...
            f_temp = TFile.Open(self.input_gen_path)
            f_temp.cd()
            h_temp = f_temp.Get(self.histo_gen_path)
            if self.do_total_cross_section == 0:
                self.h_generated = h_temp.Clone()
            else:
                self.h_generated = TH1D("generated", "generated", 1, 0, 1)
                integral, error = 0, 0
                integral = h_temp.IntegralAndError(1, h_temp.GetNbinsX(), error)
        ...
        if self.ignore_background == False:
            ...
            f_temp = TFile.Open(self.input_bkg_path)
            f_temp.cd()
            h_temp = f_temp.Get(self.histo_reco_path)
            if self.do_total_cross_section == 0:
                self.h_background = h_temp.Clone()
            else:
                self.h_background = TH1D("h_allBkg", "h_allBkg", 1, 0, 1)
                integral, error = 0, 0
                integral = h_temp.IntegralAndError(1, h_temp.GetNbinsX(), error)
